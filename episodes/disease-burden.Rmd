...
hospitalizations <- convolve(hosp, rev(density(onset_to_admission, 0:tail_value_admission)), type = "open")[seq_along(hosp)]
```

```{r}
tail_value_discharge <- quantile(admission_to_discharge, 0.999)
discharges <- convolve(hospitalizations, rev(density(admission_to_discharge, 0:tail_value_discharge)), type = "open")[seq_along(hospitalizations)]
