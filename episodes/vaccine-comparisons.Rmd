...
- What is the trade off of vaccination and NPIs?
...
Vaccine programs can be used to help control epidemics via direct and indirect protection from infection. Vaccination programs can target specific risk groups or be used alongside other control measures. In this tutorial we will compare different vaccination strategies using models from `{epidemics}`. 
...
For homogeneous mixing populations, there exists a target threshold of vaccination based on the basic reproduction number $R_0$ to achieve [herd immunity](../learners/reference.md#herdimmunity) .
...
The contact matrix for our example is :
...
+ vaccinating each age group at a rate 0.001,
...
Vaccinating group 3 only results in the highest epidemic peak size. Targeting group 2 results in the smallest epidemic peak size. Whereas targeting all age groups pushes the epidemic peak time later.  
...
Using model generated disease trajectories we have investigated the potential impact of different vaccination strategies. When infection has group specific risk, targeted vaccination programs may be more effective in reducing epidemic peak. 
