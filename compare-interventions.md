...
In this tutorial we are going to learn how to use the `{epidemics}` package to compare the effect of different interventions on simulated disease trajectories. We will access to social contact data with `{socialmixr}`. We'll use `{dplyr}`, `{ggplot2}` and the pipe `%>%` to connect some of their functions, so let's also call to the `{tidyverse}` package:
...
# run baseline model
output_baseline_vacamole <- model_vacamole(
...
output_intervention_vacamole <- model_vacamole(
  # population
  population = uk_population_vacamole,
  vaccination = double_vaccination,
  # intervention
...
``` r
# create intervention_type column for plotting
output_intervention_vacamole$intervention_type <- "mask mandate"
output_baseline_vacamole$intervention_type <- "baseline"
output_vacamole <- rbind(output_intervention_vacamole, output_baseline_vacamole)
...
You have been tasked to investigate the potential impact of an intervention on an Ebola outbreak in Guinea. Using `model_ebola()` and the the information detailed below, find the number of infections averted when :

+ an intervention is applied to reduce transmission rate by 0.5 from day 60 and,
+ an intervention is applied to reduce transmission by 0.1 from day 30.

For both interventions assume the transmission rate follows a normal distribution with mean = 1.1 / 12 and  standard deviation = 0.01.
...
``` output
   scenario averted_median averted_lower averted_upper
      <int>          <num>         <num>         <num>
1:        1             35         1.000       129.000
2:        2             23       -24.025       117.025
```
