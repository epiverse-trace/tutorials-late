...
This document aims to assess and enhance your understanding of the fundamental 
...
The parameter that explains the transition from the Susceptible (\( S \)) compartment 
to the Exposed (\( E \)) compartment is \underline{\hspace{6cm}}.
\item \(E\) stands for \underline{\hspace{2cm}}, meaning that it can 
    ...
    \underline{\hspace{3cm}}.
    The rate that explains the transition from  (\( I \)) to Recovered (\( R \)) is the rate of \underline{\hspace{6cm}}.
...
\begin{itemize}
    \item \textbf{Susceptible compartment:}
    \[
    \frac{dS}{dt} = 
    \]
    
    \item \textbf{Exposed compartment:}
    \[
    \frac{dE}{dt} = 
    \]
    \item \textbf{Infectious compartment:}
    \[
    \frac{dI}{dt} = \
    \]
    
    \item \textbf{Recovered compartment:}
    \[
    \frac{dR}{dt} =
    \]
\end{itemize}
...
called Measles$_r0$ that implements this formula. The function will use the following parameter values:
