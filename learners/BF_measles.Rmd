...
each one if susceptible, and can call this $\beta$ for short.  Consequently, on
average, the infected individual will infect
$\beta.\mathbf{S}$ susceptible individuals per
time unit. This represents the average rate at which
...
$\frac{1}{\omega}$ units time (the latent period) and then move to
...
The four S-E-I-R classes considered define the $\mathbf{SEIR}$ model.
...
\frac{dE}{dt} &=&\beta. \frac{S.I}{N} &-& \omega. E\\
...
At time $t_0=0$, we assume there is one infected person ($\mathbf{I}_0 =1$), while
$\mathbf{R}_0 = 0$). These are known as the initial values of the system
in [Equation 1](#eq-s1).

Solving [Equation 1](#eq-s1) involves determining the
...
initial conditions. Since the compartments represent numbers of individuals, their
...
interesting observation --It tells that the epidemic increases if new inftions
...
3.  The ratio $\frac{\beta}{\gamma}$ represents the basic reproduction
    ...
    individual's impact on transmission over their entire infectious duration.
    
     $\mathcal{R}_0$ is a crucial metric for
...
typical infected person can produce. This number is called the 
__Effective Reproduction Number__ ($\mathcal{R}$) and is given by:
...
an efficacy of 93% to 97% ([About Measles, CDC]()), making vaccination a key tool in controlling
measles outbreaks.
...
[Equation 1](#eq-s1) is commonly employed to simulate the
...
According to recent estimates ([United Nations, 2023](https://population.un.org/wpp/); [Central Intelligence Agency, 2023](https://www.cia.gov/the-world-factbook/countries/burkina-faso/); [World Bank, 2023](https://data.worldbank.org/indicator/SP.POP.TOTL?locations=BF)), the age structure
...
This estimate of $\mathcal{R}_{eff}$ more accurately reflects  transmission potential.
...
data_baseline <- epidemics::new_infections(output, by_group = TRUE)
...
ggplot(data_combined) +
  ...
  labs(
    ...
    caption =
      expression(
        paste(
          "New infections under baseline and 20% reduced transmission rate (",
          beta,
...
```{r}
# Load new infections data for each scenario
data_vaccine <- epidemics::new_infections(output3, by_group = TRUE)
